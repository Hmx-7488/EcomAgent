<template>
  <section>
    <div class="page-heading"><div><div class="eyebrow">事实优先 / Content package</div><h1>内容素材工作台</h1><p>每个内容包绑定已批准商品事实版本；批准版本只读，导出仅在批准后开放。</p></div><el-button type="primary" @click="create">新建内容包</el-button></div>
    <el-alert v-if="error" type="error" :title="error" :closable="false" show-icon class="notice" />
    <div class="layout">
      <aside class="package-list panel" v-loading="loading"><div class="list-label">内容包 / {{ packages.length }}</div><button v-for="item in packages" :key="item.id" class="package-row" :class="{active:selected?.id===item.id}" @click="selected=item"><span>{{ item.product_name }}</span><small>v{{ item.version }} · {{ statusText(item.status) }}</small></button></aside>
...
import { m2Api, type ApprovalStatus, type ContentPackage } from "../api/milestone2";
const packages=ref<ContentPackage[]>([]), selected=ref<ContentPackage>(), loading=ref(false), working=ref(false), error=ref(""), tab=ref("title");
...
async function refresh(){loading.value=true;try{packages.value=await m2Api.listPackages();if(!selected.value)selected.value=packages.value[0]}catch(e){error.value=errorMessage(e)}finally{loading.value=false}}
async function create(){working.value=true;try{const item=await m2Api.createPackage(1);await refresh();selected.value=packages.value.find(p=>p.id===item.id);ElMessage.success("已创建内容包草稿")}catch(e){error.value=errorMessage(e)}finally{working.value=false}}
