<template>
  ...
  <el-container v-else class="shell">
    <aside class="sidebar">
      ...
      <div class="side-note">P0 记录已批准事实、人工确认素材与售前审核记录。不含平台发布、投放、调价或售后裁决。</div>
    </aside>
...
const menu = computed(() => {
  if (!auth.user) return [];
  if (auth.user.role === "customer_service") return [{ to: "/service", label: "售前客服" }];
  const items = [{ to: "/products", label: "商品事实" }, { to: "/content", label: "内容素材" }, { to: "/image-tasks", label: "图片任务" }];
  if (auth.user.role === "admin") items.push({ to: "/approvals", label: "审批中心" }, { to: "/service", label: "售前客服" });
  return items;
});
const title = computed(() => route.path.startsWith("/products") ? "商品工作台" : route.path.startsWith("/content") ? "内容素材工作台" : route.path.startsWith("/image-tasks") ? "图片任务" : route.path.startsWith("/approvals") ? "审批中心" : route.path.startsWith("/service") ? "售前客服工作台" : "EcomAgent");
