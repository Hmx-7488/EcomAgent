...
| M2-QA-002 | Open | Low | 默认回归 warning | Pydantic class-based Config、`model_name` protected namespace 与 UTC naive datetime 仍产生 6 项 warning；不影响 M2 功能，但应在后续维护窗口迁移 `ConfigDict`、配置 `protected_namespaces` 并改用 timezone-aware UTC。 | 质量维护 |
